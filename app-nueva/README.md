...
4. Si necesitas auth: copia `auth.py` desde cualquier aplicación existente y usa `Depends(verify_token)` en las rutas.
5. Si necesitas DB: crea `models/` y `dao/` como en app-tiktok, y usa SQLite en `/app/data/nombre.db`.
6. Ejecuta `docker compose up -d` en la carpeta de la app. Traefik la detectará automáticamente.
